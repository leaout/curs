...
def init(context):
    ...
    context.data_file = os.path.join(context.data_dir, 'strategy_records/constant_limit_up_strategy_trades.json')
    # 加载历史数据
...
# 1739170800000
        ...
        if 'ST' in stock_name or 'st' in stock_name:
            continue
        # 过滤昨日没有涨停的股票
        if context.pre_limit_up_stocks and stock_code not in context.pre_limit_up_stocks:
            continue
...
def buy_at_limit_up(context, stock_code, price):
    ...
    if buy_volume > 0:
        ...
        signal_saved = db_manager.save_strategy_signal(
            strategy_name='constant_limit_up_strategy',
...
def after_trading(context):
    ...
    context.account.save_daily_account_info('constant_limit_up_strategy')
